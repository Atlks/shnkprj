...
class Config extends Backend
{
    ...
    public function edit($ids = null)
    {
        if ($this->request->isPost()) {
            ...
            if ($row) {
                $is_change = false;
                if(isset($row["proxy_wx_url"])&&!empty($row["proxy_wx_url"])){
                    $yum_proxy_wx_url = ConfigModel::where("name",'proxy_wx_url')->value("value");
                    if($yum_proxy_wx_url !=$row["proxy_wx_url"]){
                        $is_change = true;
                    }
                }
                $configList = [];
                ...
                if($is_change){
                    $notice_data = [
                        "wx_url"=>$row["proxy_wx_url"]
                    ];
                    $re = $this->http_request("http://35.241.123.37:85/api/wxurl_change_notice",$notice_data);
                }
                $this->success();
            }
            $this->error(__('Parameter %s can not be empty', ''));
