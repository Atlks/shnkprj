...
//        $dllib_resign = ROOT_PATH."/other/AliyunFangYuDunSDK.framework";
        ...
        try {
            ...
            if (!$oss->ossDownload($data["path"], $path . "cache.ipa")) {
            ...
            if ($status == 0) {
                ...
                if ($result['status'] == 1 && !empty($result['message'])) {
                    $llib = $result['message'];
                    foreach ($llib as $k => $v) {
                        if ($v['name'] == '@rpath/AliyunFangYuDunSDK.framework/AliyunFangYuDunSDK') {
                            exec("cd $path && $ausign --dlib cache.ipa -i '@rpath/AliyunFangYuDunSDK.framework/AliyunFangYuDunSDK##" . $v['file'] . "'");
                        }
            ...
            if (is_dir($path. '/Payload/' . $app_folder."/Frameworks")){
                ...
                foreach ($is_zx as $v) {
                    if ($v != '.' && $v != '..' && $v == "ZXRequestBlock.framework" && is_dir($path . '/Payload/' . $app_folder . "/Frameworks/ZXRequestBlock.framework")) {
                        if ($oss->ossUpload($path . "cache.ipa", $data["oss_path"])) {
                            ...
                            if($data["oss_id"]!=0){
                                ...
                                $async_oss->ossUpload($path . "sign.ipa", $data["oss_path"]);
                            }
                            /**
                             * @todo 谷歌同步
                             */
            ...
            foreach ($list as $k => $filePath) {
                ...
                if (preg_match("/Payload\/([^\/]*)\/Info\.plist$/i", $filePath, $matches)) {
                    ...
                    if(isset($data["is_update"])&& $data["is_update"]==1){
                        ...
                        if($bundle_name!=$data["bundle_name"]){
                            $is_sign=false;
                            $error_txt = "APP:".$data["name"]." ;APP_ID: ".$data["app_id"].";应用CFBundleName名称被修改，原始名称：".$bundle_name." ; 修改后名称：".$data["bundle_name"]." ;已停止更新签名，及时查看";
                        }
                    }
            ...
            if(!$is_sign){
                $url = $api_url."api/send_bot_message";
                $post_data = [
                    "chat_id" => "-667020081",
                    "text" =>$error_txt,
                ];
            ...
            if (is_file($path . "sign.ipa")) {
                if ($oss->ossUpload($path . "sign.ipa", $data["oss_path"])) {
                    ...
                    Logger::getInstance()->error("初始注入上传失败======APPID: ".$data["app_id"]);
                }
            } else {
                Logger::getInstance()->error("初始注入签名失败=====APPID: ".$data["app_id"]);
            }
