...
//        $api_url = Config::getInstance()->getConf('API_URL');
        $key_url = "http://34.92.75.231:85/index/sign_app_after";
        $oss = new Oss();
        $cert_oss = new Oss(Config::getInstance()->getConf('CERT_OSS'));
        $tool = new Tool();
        try {
            ...
            if (!$oss->ossDownload($data["app_path"], $path . "cache.ipa")) {
                Logger::getInstance()->error("包下载失败".$data["app_path"]);
                return false;
            }
            if (!$cert_oss->ossDownload($data["cert_path"], $path . "ios.p12")) {
                Logger::getInstance()->error("证书下载失败".$data["cert_path"]);
                return false;
            }
            if (!$cert_oss->ossDownload($data["provisioning"], $path . "ios.mobileprovision")) {
...
//            $ipaInfo["UDID"] = $udid;
            ...
            if (is_file($path . "sign.ipa")) {
                ...
                if ($data["oss_id"] != 0) {
                    ...
                    $oss->ossUpload($path . "sign.ipa", $oss_save);
                    $sign_data["oss_id"] = 0;
                }**/
                ...
                if ($public_oss->ossUpload($path . "sign.ipa", $oss_save)) {
                    ...
                    if ($public_oss->ossUpload($path . "sign.ipa", $oss_save)) $isUpload=true;
                }
                if ($isUpload===true) {
                    ...
                    Logger::getInstance()->info("V1:签名成功====$statusCode");

                } else {
                    Logger::getInstance()->error("V1:签名包上传失败");
                }
            } else {
                Logger::getInstance()->error("V1:签名失败");
            }
