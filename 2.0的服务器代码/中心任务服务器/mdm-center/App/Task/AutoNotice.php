            ...
            foreach ($list as $k => $v) {
                if (array_key_exists($v["user_id"], $cache) && ($v["sign_num"] >= $cache[$v["user_id"]]["sign_num"])) {
                    ...
                    $domain = DbManager::getInstance()->invoke(function ($client) use ($pid) {
                        return User::invoke($client)->where('id', $pid)->get();
                    });
                    ...
                    $post_data = [
                        ...
                        "text" => "次数不足提醒：< " . $business . " > 用户 < " . $cache[$v["user_id"]]["username"] . " >次数不足：" . $v["sign_num"],
//                        "text" => "次数不足提醒：用户 < " . $cache[$v["user_id"]]["username"] . " >次数不足：" . $v["sign_num"],
                    ];
