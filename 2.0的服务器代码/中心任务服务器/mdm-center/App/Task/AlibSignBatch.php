...
class AlibSignBatch implements TaskInterface
{
    ...
    public function run(int $taskId, int $workerIndex) 
    {
        ...
        DbManager::getInstance()->invoke(function ($client){
            $this->account = Enterprise::invoke($client)
                ->where('id', $this->taskData['account_id_new'])
                ->field('id,oss_path,oss_provisioning,password')
                ->get();
        ...
        //已完成提交签名任务条数
        $taskNum = 0;
        Logger::getInstance()->info('提交批量linux签名任务开始：'.$this->taskData['task_key']);
        ...
        while (true){
            ...
            $appList = DbManager::getInstance()->invoke(function ($client) use (&$pageNum,&$appId) {
                $resList = App::invoke($client)
                    ...
                    ->where('account_id',$this->taskData['account_id_old']);
                    // 根据用户使用量再做一次筛选
                    $flag= "";
                ...
                return $resList->order('id', 'ASC')
                    ->limit(0, $pageNum)
                    ->field('id,oss_path,user_id,update_time,package_name,is_resign,tag')
                    ->all();
        ...
        Logger::getInstance()->info('提交批量linux签名任务完成：'.$this->taskData['task_key'].'====总数量：'.$taskNum);
        return true;
    }
            ...
            go(function () use (&$wait,&$v,&$res){
                ...
                if(!$isGoogle){
                    ...
                    if (!$isGOss) {
                        ...
                        //检查国内私有云是否存在文件
                        $isOss = $this->oss->is_exit($v->oss_path);
                        if (!$isOss) {
                            Logger::getInstance()->info('国内OSS暂无资源，请先点击 包同步查询，APPID：'.$v->id);
                        }else{
                            $res[]=$v;
                        }
                        $isOss=null;
                ...
                if (!$isGoogle && !$isGOss) {
                    Logger::getInstance()->info('OSS暂无资源，请重新上传IPA，APPID：'.$v['id']);
                }*/
                $wait->done();
            });
        }
    ...
    private function handSignTask($appList)
    {
        ...
        foreach($appList as &$v){
            ...
            try{
                RedisPool::invoke(function (Redis $redis) use (&$ansycData) {
                    $redis->select(8);
                    $redis->rPush('linux_sign_list_task', json_encode($ansycData));
                });
            }catch(\Throwable $e){
                Logger::getInstance()->error('linux签名加入待提交列表:'.$e->getMessage());
            }
            $ansycData=null;
            $num+=1;      
        }
        $appList=null;
        $v=null;
        if($num>0) Logger::getInstance()->info('提交linux签名任务：'.$this->taskData['task_key'].'，数量：'.$num);
        return $num;
    }
