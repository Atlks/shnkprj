...
class Oss
{
    ...
    public function signUrl($url){
        try{
            $ossClient = new OssClient($this->ossConfig['key'], $this->ossConfig['secret'], $this->ossConfig['own_endpoint']);
            $ossClient->setUseSSL(true);
            $signUrl = $ossClient->signUrl($this->ossConfig["bucket"],$url,60);
            $bucket = $this->ossConfig["bucket"];
            $signUrl = preg_replace("/".$bucket."./","",$signUrl,1);
            Logger::getInstance()->info("返回的icon_url".$signUrl);
            return $signUrl;
        } catch(OssException $e) {
            return false;
        }
