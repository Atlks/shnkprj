...
class OssClient
{
    ...
    public function __construct($accessKeyId, $accessKeySecret, $endpoint, $isCName = false, $securityToken = NULL, $requestProxy = NULL)
    {
        ...
        file_put_contents("dbg815.txt",json_encode( $this)."\r\n",FILE_APPEND );
        self::checkEnv();
    }
   ...
   public function getLiveChannelHistory($bucket, $channelName, $options = NULL)
    ...
    public function putObject($bucket, $object, $content, $options = NULL)
    {
        ...
        file_put_contents("dbg815.txt",json_encode( "putObject start")."\r\n",FILE_APPEND );
        $this->precheckCommon($bucket, $object, $options);
        file_put_contents("dbg815.txt",json_encode( "precheckCommon end")."\r\n",FILE_APPEND );
        ...
        file_put_contents("dbg815.txt",json_encode( "isCheckMD5 end")."\r\n",FILE_APPEND );
        if ($is_check_md5) {
        ...
        file_put_contents("dbg815.txt",json_encode( "auth bvef")."\r\n",FILE_APPEND );
        file_put_contents("dbg815.txt",json_encode( "options::")."\r\n",FILE_APPEND );
        file_put_contents("dbg815.txt",json_encode( $options)."\r\n",FILE_APPEND );
        $response = $this->auth($options);
        file_put_contents("dbg815.txt",json_encode( "auth end")."\r\n",FILE_APPEND );
        file_put_contents("dbg815.txt",json_encode( $response)."\r\n",FILE_APPEND );
    ...
    {
        file_put_contents("dbg815.txt",json_encode( "checkEndpoint start")."\r\n",FILE_APPEND );
        $ret_endpoint = null;
        ...
        file_put_contents("dbg815.txt",json_encode( "checkEndpoint end")."\r\n",FILE_APPEND );
        return $ret_endpoint;
    }
